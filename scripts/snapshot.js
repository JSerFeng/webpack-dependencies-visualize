...
const copyForSnapshot = (sourcePath, destinationPath) => {
  ...
  if (stats.isSymbolicLink()) {
    ...
    if (resolvedStats.isDirectory()) {
      ...
      for (const entry of fs.readdirSync(resolvedPath)) {
        copyForSnapshot(path.join(resolvedPath, entry), path.join(destinationPath, entry));
      }
      return;
    }
  ...
  if (stats.isDirectory()) {
    ...
    for (const entry of fs.readdirSync(sourcePath)) {
      ...
      copyForSnapshot(path.join(sourcePath, entry), path.join(destinationPath, entry));
    }
    return;
  }
