...
import { destroyWebContainer, initWebContainer } from './utils/webContainer';
import { type FileMap, compileCode } from './utils/webpackCompiler';
...
function App() {
  ...
  useEffect(() => {
    ...
    if (encodedFiles) {
      try {
        const filesJson = decodeURIComponent(
          escape(atob(decodeURIComponent(encodedFiles))),
        );
